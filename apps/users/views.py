...
def create_api_key(request):
    ...
    messages.success(
        request,
        _('API Key created. Your key is: %(key)s. Save this somewhere safe - you will only see it once!') % {
            'key': key,
        }
    )
...
def revoke_api_key(request):
    ...
    messages.success(
        request,
        _('API Key %(key)s has been revoked. It can no longer be used to access the site.') % {
            'key':api_key.prefix,
        }
    )
    return HttpResponseRedirect(reverse('users:user_profile'))
